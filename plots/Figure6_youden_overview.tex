...
\thispagestyle{empty}
% plot style
\newcommand{\plotdir}{s5_results_I_wadd-1}
\newcommand{\plotstyle}{Y_overview_all-binary} 
\newcommand{\datachoice}{cafpas} 
\newcommand{\append}{_uniform}

\newcommand{\modelA}{all-models_Y90_wadd-1}  
...
% ...
% measurements/subjective information - hier noch Reihenfolge ändern 
...
% plot parameters
\newcommand{\xstart}{3}
\newcommand{\deltax}{3.9} % 3.2 an sich, fuer weights 3.5
\newcommand{\deltaxII}{4}
\newcommand{\ystart}{15}
\newcommand{\deltay}{-4.1} %-3.9
\newcommand{\trimleft}{0.73} % 0.72 an sich, fuer weights 0.65
...
% to do: 
% - Spalten 3-5 mit weiteren Vergleichen füllen 
% - evtl a) b) c) etc. - Nummerierung für Paper
...
\begin{tikzpicture}[remember picture, overlay] 
%% Legenden Kategorienvergleich (Spaltentitel) - könnte man durch tex-Schrift ersetzen (Linien hier nicht benötigt)
...
{
\node[NS3,\st] (c1) at ({\xstart+(\i-1)*\deltax-0.23},\ystart+3.0){Comparison \\ set \setno \\ \vspace{0.1cm} \scriptsize \texta \\ \vspace{-0.3cm} \textb}; %  \scriptsize (single)/joint 
}
...
{\node[inner sep=0pt,scale=1] (russell) at ({\xstart-0.37+(\i-1)*\deltax},{\ystart+0.2+(\j-1)*\deltay}){\includegraphics[trim={0cm 0 0 0.2},clip]{./\plotdir/\plotstyle_\comp_\model}}; 
}
\foreach \i/\comp in {2/\compB,3/\compC,4/\compD,5/\compE} % Zeile 
{ \node[inner sep=0pt,scale=1] (russell) at ({\xstart+(\i-1)*\deltax},{\ystart+0.2+(\j-1)*\deltay}){\includegraphics[trim={\trimleft cm 0 0 0},clip]{./\plotdir/\plotstyle_\comp_\model}};    
}
}

%\foreach \j/\meas/\tl/\tb/\tr/\tt in {1/\measA/1.9/1.2/0.5/0.5,2/\measB/1.9/1.2/0.5/0.5,3/\measC/1.9/1.2/0.5/0.5,4/\measD/1.8/1.1/0.5/0.5,5/\measE/2/1.2/0.5/1.5,6/\measF/2/1.2/0.5/1.5} % Legenden individuell getrimmt (auch abhängig von \datachoice)
...
% 1. Zeile manuell Kategorien (zwischen Überschrift Comparison set und Plots)
